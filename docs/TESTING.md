...
- `tests/master`: ACR parsing, ACR token cache lifecycle, execution client authorization, module source policy, gateway body limits, gateway errors, method enforcement, request ID propagation and idempotency, durable SQLite job storage/replay/recovery/querying, idempotency cache eviction and stats, idempotency metrics, dispatcher errors and conservative rescheduling, worker draining, registry stale-timeout cleanup, scheduler capacity/state filtering.
