...
- `tests/master`: ACR parsing, ACR token cache lifecycle, execution client authorization, module source policy, gateway body limits, gateway errors, method enforcement, request ID propagation and idempotency, durable SQLite job storage/replay/recovery/querying, async durable job creation, idempotency cache eviction and stats, idempotency metrics, dispatcher errors and conservative rescheduling, worker draining, registry stale-timeout cleanup, scheduler capacity/state filtering.
...
- `tests/worker`: WASM engine execution, module digest verification, error classification and metrics, cache lifecycle, worker limits including shutdown defaults, worker health/readiness, method enforcement, JSON error handling.
