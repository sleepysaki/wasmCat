...
- `tests/bootstrap`: config file generation, overwrite protection, positive duration validation, and local cert generation.
- `tests/config`: environment parsing, default derivation, and validation, including positive master/worker timeout settings.
- `tests/master`: ACR parsing, ACR token cache lifecycle, execution client authorization, module source policy, gateway body limits, gateway errors, method enforcement, request ID propagation and idempotency, durable SQLite job storage/replay/recovery/querying, async durable job creation, worker job completion recording, idempotency cache eviction and stats, idempotency metrics, dispatcher errors and conservative rescheduling, worker draining, registry stale-timeout cleanup, scheduler capacity/state filtering.
...
- `tests/worker`: WASM engine execution, module digest verification, error classification and metrics, cache lifecycle, job completion callbacks, worker limits including shutdown defaults, worker health/readiness, method enforcement, JSON error handling.
