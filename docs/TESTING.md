...
- `tests/master`: ACR parsing, ACR token cache lifecycle, execution client authorization, module source policy, gateway body limits, gateway errors, method enforcement, request ID propagation, dispatcher errors, worker draining, registry stale-timeout cleanup, scheduler capacity/state filtering.
