...
WebAssembly's main contribution to this design is its sandboxed execution model. A module cannot directly access host memory; it interacts with the host through exported and imported functions and through linear memory. wasmCat defines a simple custom ABI in which modules export `malloc` and `run`. The host calls `malloc` to reserve module memory for the input payload, writes the payload bytes into WebAssembly linear memory, then calls `run(ptr, len)`. The module returns a packed pointer and length describing the output region. This custom ABI is simple enough for a thesis prototype, while the worker additionally supports standard WASI (`wasip1`) modules that communicate over standard input and output, so general-purpose modules compiled from common toolchains can run without targeting a wasmCat-specific interface.
...
The worker uses wazero as an embedded WebAssembly runtime. `NewWasmEngineWithLimits` creates a wazero runtime with close-on-context behavior, initializes a compiled module cache, initializes an in-flight compile map, creates an HTTP client, normalizes execution limits, and creates a semaphore channel sized by `MaxConcurrentExecs`. It also instantiates the `wasi_snapshot_preview1` host module into the runtime, so standard WASI modules can execute alongside custom-ABI modules; a custom-ABI module simply does not import these functions.

Execution begins in `ExecuteWithDigestAndABI`, which `ExecuteWithDigest` wraps with the default auto-detect mode. The worker first rejects oversized payloads. It then tries to acquire the semaphore. If all execution slots are busy, the function returns a capacity error instead of queueing unbounded work in memory. After acquiring a slot, the worker creates an execution context with `ExecutionTimeout`. This timeout covers module fetching, compilation, instantiation, the mode-specific invocation, output reading, and cleanup.
...
The worker supports two execution modes, selected by an optional `abi` field on the request (`wasmcat`, `wasi`, or empty for auto-detection). When the field is empty, the worker inspects the compiled module's exported functions: a module that exports `run` is executed with the custom ABI, and a module that exports `_start` is executed as a WASI module.

In the custom wasmCat ABI, the module exports `memory`, `malloc`, and `run`. The host calls `malloc(size)` to obtain a pointer in module memory, writes the input bytes at that pointer using `mod.Memory().Write`, and calls `run(ptr, len)`. The result is a `uint64` in which the high 32 bits represent the output pointer and the low 32 bits represent the output length. The host checks the length against `MaxOutputBytes`, reads the output bytes from linear memory, and returns them as a Go string.

In WASI mode, the worker runs a standard `wasip1` command module produced by toolchains such as Rust, Go, TinyGo, or C without any wasmCat-specific code. The worker delivers the payload on standard input, executes the module's `_start` entry point against the `wasi_snapshot_preview1` host, and reads the result from standard output; a WASI exit code of zero is treated as success, and the output is bounded by the same `MaxOutputBytes` limit. Both modes share the digest-aware compiled-module cache and the per-request instantiation model, so each invocation receives a fresh instance with isolated linear memory regardless of mode. The current boundary is that a WASI module may import only `wasi_snapshot_preview1`; modules requiring additional host interfaces are not yet supported.
...
The custom ABI is simple and transparent. The host and module exchange strings through linear memory using pointer and length values, which makes the memory-safety boundary explicit and is useful for an academic prototype. In this mode the ABI is strict: a module that does not export `malloc`, `memory`, or `run` will fail, an invalid pointer or length causes a read error, and output larger than the configured limit is rejected. To run general-purpose code without targeting this interface, the worker also supports standard WASI (`wasip1`) modules, which exchange data over standard input and output; this broadens the range of production code the platform can execute. The remaining limitation is that a WASI module may import only `wasi_snapshot_preview1`. Future versions should define a more formal custom-ABI specification and include tooling to validate modules before deployment.
...
The fifth direction concerns module compatibility and tooling. The worker already supports both the custom `malloc`/`run` ABI and standard WASI (`wasip1`) modules, so general-purpose code can run today. Building on this, a production platform should provide a module SDK, validation tooling, and versioned ABI contracts, and should extend WASI support to workloads that require host interfaces beyond `wasi_snapshot_preview1`.
...
Answer: The worker supports two modes: the custom `malloc`/`memory`/`run` ABI and standard WASI (`wasip1`) modules that use standard input and output. The custom ABI is simple and efficient but requires modules to follow its exact contract, while WASI accepts general-purpose modules as long as they import only `wasi_snapshot_preview1`. The biggest remaining limitation is therefore modules that need host interfaces beyond that, together with the absence of SDK tooling and versioned ABI definitions, which are future work.
