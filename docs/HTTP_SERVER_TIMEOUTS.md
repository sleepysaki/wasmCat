...
Graceful shutdown is separate from per-connection timeouts. `MASTER_SHUTDOWN_TIMEOUT` controls how long the master waits for in-flight HTTP requests during shutdown. `WORKER_SHUTDOWN_TIMEOUT` does the same for worker HTTP requests; execution itself is still capped by `EXECUTION_TIMEOUT`.
