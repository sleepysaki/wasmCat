...
7. If dispatch fails, the durable job records `failed` and `last_error`. Workers also report final success or failure through `/internal/jobs/complete`, giving the master a second persistence path for completed work.
